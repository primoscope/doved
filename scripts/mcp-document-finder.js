    ...
    async validateEnvFile(content, validation) {
        ...
        for (const line of lines) {
            ...
            if (trimmed && !trimmed.startsWith('#')) {
                if (trimmed.includes('=')) {
                    varCount++;
                    // Enhanced security validation for secrets
                    const secretVarRegex = /^(?:\s*)((?:DB_|MYSQL_|POSTGRES_|API_|APP_|USER_|)[A-Za-z0-9_\-]*(?:PASSWORD|PASSWD|PWD|SECRET|TOKEN|KEY|CREDENTIALS))\s*=\s*([^\s#]+)$/i;
                    const match = trimmed.match(secretVarRegex);
                    if (match) {
                        ...
                        if (!isPlaceholder) {
                            validation.issues.push(`Possible hardcoded secret in line: ${trimmed.substring(0, 50)}`);
                        }
                    }
                } else {
