...
# Enhanced Health Check Script for EchoTune AI Production Deployment
...
# Enhanced SSL certificate check with chain validation
check_ssl_certificate() {
    ...
    if command_exists openssl; then
        local cert_info
        # Enhanced SSL check with hostname verification and chain validation
        cert_info=$(echo | openssl s_client -servername "$domain" -connect "$domain:443" -verify_return_error -verify_hostname "$domain" 2>/dev/null | openssl x509 -noout -dates 2>/dev/null || echo "FAILED")
